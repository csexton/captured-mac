...
- (id)initWithKeychainUsingAppName:(NSString *)name serviceProviderName:(NSString *)provider 
{
    ...
    if (status == noErr) {
        //self.key = [NSString stringWithCString:list.attr[0].data length:list.attr[0].length];
        self.key = [[NSString alloc] initWithBytes:list.attr[0].data length:list.attr[0].length encoding:NSASCIIStringEncoding];
        if (password != NULL) {
			...
			self.secret = [NSString stringWithCString:passwordBuffer encoding:NSASCIIStringEncoding];
        }
